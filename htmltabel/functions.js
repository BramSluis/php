...
function fGetHtml(users) {                  
    ...
    htmlTable += "</tr>";
    for (var i = 0; i < users.length; i++) { // loop door alle users plaats de row met headers van de tabel
        htmlTable += "<tr>";
...
function getHtmlFromUrl(url) {  // haal dat van URL binnen, verwerkt dit in een HTML-table en plaatst {
    fetch(url)
        ...
        .then(json => {
            ...
            console.log("users = ", json.data);        // de json bevat users en offices, laat hier alleen de users zien 
